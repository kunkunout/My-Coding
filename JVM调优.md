- [JVM 调优](#jvm-调优)
  ...
  - [堆内存模型](#堆内存模型)
    ...
    - [YoungGC 和 OldGC](#younggc-和-oldgc)
    - [JVM 运行监控工具VisualVM](#jvm-运行监控工具visualvm)
  - [垃圾回收机制](#垃圾回收机制)
    - [如何判断一个对象是否是垃圾？](#如何判断一个对象是否是垃圾)
...
- **YoungGC**
  新生代区域的垃圾回收称之为YoungGC，也叫**MinorGC**，Eden区满了之后会触发YoungGC
- **OldGC**
  老年代区域的垃圾回收称之为OldGC，也叫**MajorGC**，OldGC非常浪费性能（**GC完开始整理**），所以我们的JVM调优要尽可能地减少OldGC的次数，OldGC往往伴随着YoungGC。**YoungGC + OldGC = FullGC**  

  问题：  
- 1. Survivor区空间并不大，如果满了怎么办？  
  - 一般情况下GC会回收95%的对象，且超过15次的GC对象会存放到Old区，所以Survivor区不容易满。  
  - 如果Survivor区满了，会触发担保机制，提前将对象存入Old区。
- 2. 为什么需要Survivor区？
...
### JVM 运行监控工具VisualVM
打开Java安装目录**/bin/jvisualvm.exe**，安装**VisualGC**插件。  
注：**JDK9**之后该工具已经被移除，需要自行下载该工具 [下载地址](https://visualvm.github.io/index.html)  
修改堆内存大小指令：**-Xms10M**：设置初始堆内存大小为10M、**-Xmx10M**：设置最大堆内存大小为10M  
![VisualVM图形化页面](./img/image3.png)

## 垃圾回收机制
堆内存模型设计是为了提高垃圾回收的效率，那么如何判断一个对象是否是垃圾呢？

### 如何判断一个对象是否是垃圾？
- 引用计数法：
  如果要操作对象，必须通过引用来进行。如果一个对象没有任何引用与之关联，则说明该对象基本不太可能在其他地方被使用到。那么这个对象就是垃圾可以被GC。这种方法实现简单，效率高，但是它**无法解决循环引用**的问题，因此在Java中并没有采用这种方式（**Python采用引用计数法**）
- 可达性分析
